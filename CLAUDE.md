...
### 보스 3종 (정의서 11.1 / 3.7) — 완료
- **모델**: `BOSS_WAVES`(10/20/30)·`BOSS_DEF`. `G.boss`(hp·hpMax·phase·기믹상태). `startWave`가 보스화(예산0), 처치=클리어.
  `update`에 `updateBoss`/`collideBoss`, `render`에 `drawBoss`. 웨이브 클리어 게이트에 `!G.boss`.
- **WALL_E**(10): 공전 + 3 회전방패 호(`walleBlocks`로 접근각 차단). 틈으로만 피해. → 슈퍼 정비궤도.
- **ANTI_V**(20): 상단 부유, 포탑(슈터) 주기 스폰 + 조준방향 에러블록(`b.blocks`, 그 각도 탄 차단). → 슈퍼 정비궤도.
- **KERNEL_PANIC**(30): `bossBorderR()` 테두리=HP(닿는 탄이 피해), 4속성 무제한 물량. 처치=`endGame(true)` 클리어.
- **페이즈2**(50% HP) 기믹 강화. **노바가 보스에도 큰 피해**(`fireNova`, 8.1 글리치 저격, 방패무시).
- **슈퍼 정비궤도**(3.7): 중간 보스 처치 → `enterOrbit(true)` → 후보 6개·`orbitPicksLeft`=2 연속 픽(`finishMerge`가 다음 픽으로).
- **HP 밸런스**(`python tools/eval_boss.py` 실측, 레거시·결정적 빌드): PANIC 2600(스윕 17s 화력쇼), ANTI_V 351(28s),
  WALL_E 85+틈확대(ARC 0.7). WALL_E는 **정렬 강제 보스** — 3슬롯 자동스윕 격파 불가(방패), 브레이크 정렬 시 ~4.5s(의도).
- ⚠️ 이어하기(9.5)·보스 정밀 패턴 미구현. eval_boss는 자동스윕 기준선(정렬 스킬 0=상한 TTK).
...
2. 보스 정밀화 (이어하기 9.5 연동 · 패턴/페이즈 확장 · HP/밸런스 측정) — 3종 1차는 완료
3. Affix 확장 (정식 풀 10+10 + 필드 드롭/LV4 해금 + 딥웹 '품질 잡팟' 배선) — MVP는 완료
4. 아웃게임 잔여 (이어하기 9.5·링크 배치학 9.2) — 풀 1차·슈퍼 정비궤도는 완료
