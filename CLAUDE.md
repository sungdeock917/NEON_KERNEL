...
- **시그니처 공격 = 정렬 보상형 텔레그래프**(2026-06 확정): 예고글로우 후 **중심 인바운드 탄**(`mkInbound`, `boss:true`) → 브레이크 6방향 고정사격(2.1)이 그 각도를
  자동 요격(기존 1699~1715 상쇄 로직 재사용, 새 충돌코드 0). "탁으로 위상 맞춰→꾹으로 요격" 루프. 슬롯 수↓=격자 성김=난도↑. 수축 링엔 항상 틈(노히트 보장, 11.2).
- **페이즈2**(50% HP): 기믹 강화 + 공격주기 `bossAtkPeriod`×0.62 + 진입 직후 강화공격 예고. **노바가 보스에도 큰 피해**(`fireNova`, 8.1, 방패무시).
- **슈퍼 정비궤도**(3.7): 중간 보스 처치 → `enterOrbit(true)` → 후보 6개·`orbitPicksLeft`=2 연속 픽.
- **HP 밸런스**(`python tools/eval_boss.py` 실측, 레거시·결정적 빌드, TTK는 보스→플레이어 신패턴 무관): PANIC 2600(스윕 17s), ANTI_V 351(28s),
  WALL_E 85+틈확대(ARC 0.7, **정렬 강제** — 3슬롯 자동스윕 격파 불가, 브레이크 정렬 ~4.5s). 신패턴 위협(자동스윕 피해입음) walle 88/antiv 8/panic 11=요격으로 방어가능 확인.
- ⚠️ eval_boss는 자동스윕 기준선(정렬 스킬 0=상한 TTK). 보스 정밀 패턴 추가 여지(3페이즈·고유 탄막)는 후속.
